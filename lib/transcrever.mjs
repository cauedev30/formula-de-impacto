...
export async function transcrever(blob) {
  ...
  if (!resposta.ok) {
    // Só os códigos que a Function devolve para o próprio áudio (vazio, grande demais,
    // silêncio): repetir dá o mesmo erro. 401/403/404 de proxy ou deploy pela metade passam.
    const erro = new Error(dados.erro || `falha ${resposta.status}`);
    erro.definitivo = [400, 413, 422].includes(resposta.status);
    throw erro;
  }
  // Portal de wifi responde 200 com HTML: sem esta checagem "undefined" virava a transcrição.
  if (typeof dados.texto !== "string") throw new Error("resposta sem texto");
  return dados.texto;
}
...
export function juntarTranscricao(valor, texto) {
  ...
  return {
    ...
    texto: atual ? `${atual}\n\n${texto}` : texto,
    transcrito: texto,
    transcritoEm: new Date().toISOString(),
  };
}

// Regravar descarta a transcrição do áudio velho, mas só se ela não foi corrigida: texto
// editado não dá para separar do digitado, e perder o que ele escreveu é pior.
export function semTranscricaoAntiga(valor) {
  const { transcrito, transcritoEm, ...resto } = valor ?? {};
  const texto = String(resto.texto ?? "");
  if (!transcrito || !texto.includes(transcrito)) return valor ?? {};
  return { ...resto, texto: texto.replace(transcrito, "").trim() };
}
